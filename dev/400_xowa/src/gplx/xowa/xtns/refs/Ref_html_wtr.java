...
public class Ref_html_wtr {
	...
	public void Xnde_references(Xoh_hdom_wtr wtr, Xop_ctx ctx, Xoh_html_wtr_ctx opts, Bry_bfr bfr, byte[] src, Xop_xnde_tkn xnde) {
...
//				if (head_itm.Exists_in_lnki_title()) continue;	// EX: if ref is in lnki_title only, ignore; EX:w:UK; DATE:2014-03-05
			...
			if (text_itm.Body() != null)
				wtr.Write_tkn(tmp, ctx, opts, text_itm.Body().Root_src(), null, Xoh_hdom_wtr.Sub_idx_null, text_itm.Body());
			...
			for (int k = 0; k < related_len; k++) {
				...
				if (related_itm.Follow_y()) {	// NOTE: both follow and related are in the related list; only add follow
					...
					wtr.Write_tkn(tmp, ctx, opts, related_itm.Body().Root_src(), null, Xoh_hdom_wtr.Sub_idx_null, related_itm.Body());
				}
			}
