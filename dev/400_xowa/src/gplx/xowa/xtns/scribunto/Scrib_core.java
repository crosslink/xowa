...
public class Scrib_core {
	...
	public OrderedHash Cur_frame_dynamic_list() {return cur_frame_dynamic_list;} private OrderedHash cur_frame_dynamic_list = OrderedHash_.new_();
	...
	public void Invoke(Xow_wiki wiki, Xop_ctx ctx, byte[] src, Xot_invk owner_frame, Xot_invk invoke_frame, Bry_bfr bfr, byte[] mod_name, byte[] mod_text, byte[] fnc_name) {
		lib_mw.Invoke_bgn(wiki, ctx, src);
		try {
			cur_frame_owner = owner_frame; this.wiki = wiki; this.ctx = ctx; this.cur_src = src;
			owner_frame.Scrib_frame_tid_(Scrib_frame_.Tid_owner); invoke_frame.Scrib_frame_tid_(Scrib_frame_.Tid_current);
			...
			lib_mw.Invoke_end();
			owner_frame.Scrib_frame_tid_(Scrib_frame_.Tid_null); invoke_frame.Scrib_frame_tid_(Scrib_frame_.Tid_null);
			cur_frame_dynamic_list.Clear();
		}
	}
