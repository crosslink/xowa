...
public class Scrib_frame_ {
	public static final byte Tid_null = 0, Tid_current = 1, Tid_owner = 2, Tid_dynamic = 3;
	...
	public static int Get_frame_tid(String frame_id) {
		...
		else if (String_.Eq(frame_id, "parent"))	return Tid_owner;
		else										return Tid_dynamic;
	}
	public static Xot_invk Get_frame(Scrib_core core, String frame_id) {
		if		(String_.Eq(frame_id, "current"))	return core.Cur_frame_invoke();
		else if (String_.Eq(frame_id, "parent"))	return core.Cur_frame_owner();
		else {
			Xot_invk rv = (Xot_invk)core.Cur_frame_dynamic_list().Fetch(frame_id);
	...
	public static Xot_invk Get_owner(Scrib_core core, byte frame_tid) {
		if		(frame_tid == Tid_current)	return core.Cur_frame_owner();		// current frame has an owner frame
