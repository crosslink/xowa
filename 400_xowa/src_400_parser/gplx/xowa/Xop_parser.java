...
public class Xop_parser {	// NOTE: parsers are reused; do not keep any read-write state
	public Xop_parser(Xop_lxr_mgr tmpl_lxr_mgr, Xop_lxr_mgr wtxt_lxr_mgr) {
	...
	public void Parse_text_to_html(Bry_bfr trg, Xoa_page page, boolean para_enabled, byte[] src) {
		...
		wiki.Html_mgr().Hdom_wtr().Write_all(trg, ctx, wtxt, root);
	}
	...
	private void Parse(Xop_root_tkn root, Xop_ctx ctx, Xop_tkn_mkr tkn_mkr, byte[] src, byte parse_type, ByteTrieMgr_fast trie, int doc_bgn_pos) {
		...
		Parse_loop(root, ctx, tkn_mkr, src, trie, doc_bgn_pos, len);
		ctx.Page_end(root, src, len);
		ctx.Parse_tid_(parse_tid_old);
	}
	public void Parse_loop(Xop_root_tkn root, Xop_ctx ctx, Xop_tkn_mkr tkn_mkr, byte[] src, ByteTrieMgr_fast trie, int pos, int len) {
