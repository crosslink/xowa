...
class Xoa_app_boot_mgr {
	...
	private boolean Init_env(String[] args) {
		...
		Xoa_app_.Build_date = JarAdp_.ModifiedTime_type(Xoa_app_.class).XtoUtc().XtoStr_fmt("yyyy-MM-dd HH:mm"); log_wtr.Log_msg_to_session_fmt("env.init: build_date=~{0}", Xoa_app_.Build_date);
		GfuiEnv_.Init_swt(args, "xowa.jar", Xoa_app_.class); log_wtr.Log_msg_to_session_fmt("env.init: jar url=~{0}; op_sys=~{1}", Env_.AppUrl(), Op_sys.Cur().Xto_str());
		chkpoint = "init_env";
		return true;
	}
