...
public class Pp_pages_nde implements Xox_xnde, Xop_xnde_atr_parser {
	...
	public void Xtn_write(Xoa_app app, Xoh_hdom_wtr html_wtr, Xoh_html_wtr_ctx opts, Xop_ctx ctx, Bry_bfr bfr, byte[] src, Xop_xnde_tkn xnde) {
		if (xtn_literal)
			...
			html_wtr.Write_tkn(bfr, ctx, opts, xtn_root.Root_src(), xnde, Xoh_hdom_wtr.Sub_idx_null, xtn_root);
		}
	}
	...
	private byte[] Get_caption(Bry_bfr full_bfr, byte[] index_page_src, Xop_lnki_tkn lnki) {
		byte[] rv = Bry_.Empty;
		try {
			wiki.Html_mgr().Hdom_wtr().Write_tkn(full_bfr, ctx, Xoh_html_wtr_ctx.Basic, index_page_src, null, -1, lnki.Caption_tkn());
		...
		if (main_lnkis_len > 0) {
			...
			for (int i = 0; i < main_lnkis_len; i++) {
				...
				if (page_ttl.Eq_full_db(main_lnki.Ttl())) {
					...
					wiki.Html_mgr().Hdom_wtr().Page_(ctx.Cur_page());
					...
					wiki.Html_mgr().Hdom_wtr().Page_(old_page);
					break;
				}
			}
		}
	...
	private Xop_root_tkn Bld_root_nde(Bry_bfr page_bfr, Hash_adp_bry lst_page_regy, byte[] wikitext) {
		...
		Xop_parser tmp_parser = new Xop_parser(wiki.Parser().Tmpl_lxr_mgr(), wiki.Parser().Wtxt_lxr_mgr());
