...
public class Xoh_hdom_wtr {
	...
	public void Write_all(Bry_bfr bfr, Xop_ctx ctx, byte[] src, Xop_root_tkn root) {
...
//			page.Hdrs_id_bld(hdr, src);
	...
	private void Apos(Xop_ctx ctx, Xoh_html_wtr_ctx hctx, Bry_bfr bfr, byte[] src, Xop_apos_tkn apos) {
...
//					bfr.Add(Tag__end_bgn).Add(name).Add_byte(Tag__end);		// NOTE: inline is never written as <b/>; will be written as <b></b>; SEE: NOTE_1
	...
	public void Xnde_atrs(int tag_id, Xoh_html_wtr_ctx hctx, byte[] src, int bgn, int end, Xop_xatr_itm[] ary, Bry_bfr bfr) {
		...
		for (int i = 0; i < ary_len; i++) {
			...
			Xnde_atr_write(bfr, hctx, src, atr);
		}
	}

	private void Xnde_atr_write(Bry_bfr bfr, Xoh_html_wtr_ctx hctx, byte[] src, Xop_xatr_itm atr) {
	...
	private void Xnde_atr_write_id(Bry_bfr bfr, byte[] bry, int bgn, int end) {
