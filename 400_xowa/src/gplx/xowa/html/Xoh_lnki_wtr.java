...
public class Xoh_lnki_wtr {
	...
	public Xoh_lnki_wtr(Xoh_hdom_wtr html_wtr, Xow_wiki wiki, Xow_html_mgr html_mgr, Xoh_html_wtr_cfg cfg) {
	...
	public void Page_(Xoa_page v) {
		this.page = v;
		this.wiki = v.Wiki();
		redlinks_mgr = v.Lnki_redlinks_mgr();	// NOTE: need to set redlinks_mgr, else toc parse may fail; EX:pl.d:head_sth_off;DATE:2014-05-07
		file_wtr.Page_bgn(v);
	...
	private void Write_caption(Bry_bfr bfr, Xop_ctx ctx, Xoh_html_wtr_ctx hctx, byte[] src, Xop_lnki_tkn lnki, byte[] ttl_bry, boolean tail_enabled, Xop_lnki_caption_wtr caption_wkr) {
		if	(lnki.Caption_exists()) {					// lnki has a caption seg; EX: [[A|caption]]
			if (lnki.Caption_tkn_pipe_trick())			// "pipe trick"; [[A|]] is same as [[A|A]]; also, [[Help:A|]] -> [[Help:A|A]]
				...
				caption_wkr.Write_tkn(ctx, hctx, bfr, src, lnki, Xoh_hdom_wtr.Sub_idx_null, lnki.Caption_val_tkn());
		}
...
class Xop_lnki_caption_wtr_tkn implements Xop_lnki_caption_wtr {
	private Xoh_hdom_wtr html_wtr;
	public Xop_lnki_caption_wtr_tkn(Xoh_hdom_wtr html_wtr) {
