...
public class Xog_tab_mgr implements GfoEvObj {
	...
	private void Tabs_selected(String key) {
		...
		Xog_tab_itm_read_mgr.Update_selected_tab(win.App(), win, page);
	...
	private void Tabs_closed(String key) {
		...
		if (tab_regy.Count() == 0) {
			active_tab = Xog_tab_itm_.Null;
			Xog_tab_itm_read_mgr.Update_selected_tab(win.App(), win, Xoa_page.Null);
		}
		else
			Tabs_recalc_idx();
		Tabs_hide_if_one_chk();
	...
	public void Tabs_new_link(boolean focus) {
		if (this.Active_tab_is_null()) return;
		String link = active_tab.Html_itm().Html_selected_get_text_or_href();
	...
	public Object Invk(GfsCtx ctx, int ikey, String k, GfoMsg m) {
		...
		else if	(ctx.Match(k, Invk_tabs_new_link__at_dflt__focus_n))						Tabs_new_link(Bool_.N);
		else if	(ctx.Match(k, Invk_tabs_new_link__at_dflt__focus_y))						Tabs_new_link(Bool_.Y);
