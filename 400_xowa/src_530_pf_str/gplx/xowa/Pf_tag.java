...
class Pf_tag extends Pf_func_base {
	...
	public static final byte[] Bry_tag_hdr = Bry_.new_ascii_("xowa_tag_");
	public static final int Bry_tag_hdr_len = Bry_tag_hdr.length;
	private static final byte[] Bry_tag_hdr_lhs = Bry_.new_ascii_("<xowa_tag_"), Bry_tag_hdr_rhs = Bry_.new_ascii_("</xowa_tag_");
	public void Func_evaluate2(Xop_ctx ctx, byte[] src, Xot_invk caller, Xot_invk self, Bry_bfr bfr) {
		byte[] tag_name = Eval_argx(ctx, src, caller, self); if (tag_name.length == 0) return;
		int args_len = self.Args_len();
		Bry_bfr tmp = ctx.App().Utl_bry_bfr_mkr().Get_b512();
		try {
	...
	@Override public void Func_evaluate(Xop_ctx ctx, byte[] src, Xot_invk caller, Xot_invk self, Bry_bfr bfr) {
		...
		try {
			...
			if (args_len > 0) {
				...
				content = ctx.Wiki().Parser().Parse_text_to_wtxt(sub_tkn_mkr.Root(content), sub_ctx, sub_tkn_mkr, content);	// NOTE: force reparse; needed for {{#tag:ref|{{A{{!}}B}}}}; EX: de.wikipedia.org/wiki/Freiburg_im_Breisgau; DATE:2013-12-18
				tmp.Add(content);
			}
			tmp.Add_byte(Byte_ascii.Lt).Add_byte(Byte_ascii.Slash).Add(tag_name).Add_byte(Byte_ascii.Gt);
			bfr.Add_bfr_and_clear(tmp);
	...
	boolean AddHtmlArg(byte[] src, Bry_bfr tmp) {
