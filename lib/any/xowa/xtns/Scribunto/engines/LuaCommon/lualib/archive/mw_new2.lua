...
local function newFrame( frameId, ... )
	...
	local parent_arg_keys
	local parent_arg_vals
	local current_arg_keys
	local current_arg_vals
    
  local function getAllArguments(frameId, frame_is_parent)
			local args = php.getAllExpandedArguments( frameId )
      
      if frame_is_parent then
        parent_arg_keys = {}
        parent_arg_vals = {}
      else
        current_arg_keys = {}
      ...
      local arg_idx = 1      
        ...
        if frame_is_parent then
          table.insert(parent_arg_keys, key)
          if (type(key) == 'number') then
            parent_arg_vals[tostring(arg_idx)] = val
            arg_idx = arg_idx + 1
          else
            parent_arg_vals[key] = val
          end
          parent_arg_vals[key] = val
        else
          table.insert(current_arg_keys, key)
          if (type(key) == 'number') then
            current_arg_vals[tostring(arg_idx)] = val
            arg_idx = arg_idx + 1
          else
            current_arg_vals[key] = val
          end
          current_arg_vals[key] = val
        end
			end    
	...
	local function getExpandedArgument( dummy, name )
		name = tostring( name )
    local frame_is_parent = frameId == 'parent'
    if (frame_is_parent and not parent_arg_keys) or (not frame_is_parent and not current_arg_keys) then
      getAllArguments(frameId, frame_is_parent)
    end

    local rv
    if frame_is_parent then
      rv = parent_arg_vals[name] 
    else
      rv = current_arg_vals[name] 
    end

    if (rv == nil or not rv) then
      return nil
    else
      return rv
    end      
    ...
    if (frame_is_parent and not parent_arg_keys) or (not frame_is_parent and not current_arg_keys) then
      ...
      local rv
      if frame_is_parent then
        rv = parent_arg_keys[index] 
        if rv then
				  return rv, parent_arg_vals[rv]
        end
      else
        rv = current_arg_keys[index] 
        if rv then
				  return rv, current_arg_vals[rv]
        end
      end
		end
	end
