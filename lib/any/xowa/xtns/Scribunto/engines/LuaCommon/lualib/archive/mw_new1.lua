...
local function newFrame( frameId, ... )
  ...
  local function getAllArguments(frameId)
	...
	local function getExpandedArgument( dummy, name )
		name = tostring( name )
    if (frameId == 'parent' and not parent_arg_keys) or (frameId == 'current' and not current_arg_keys) then
      getAllArguments(frameId)
    end
